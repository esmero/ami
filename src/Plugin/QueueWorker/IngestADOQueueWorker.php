...
class IngestADOQueueWorker extends QueueWorkerBase implements ContainerFactoryPluginInterface {
  ...
  public function processItem($data)
  {
    ...
    if (!isset($data->info['set_id']) || !is_numeric($data->info['set_id'])) {
      // This one is kinda key. We do have a lot of ?? further down.
      // But let's bail out of missing ok?
      $message = $this->t('Error for ami_ingest_ado queue. Any AMI Set Queue Item needs to have a defined set_id. This one has none. Did you create the queue item programmatically? Skipping.');
    ...
    */
      ...
      if (isset($data->pluginconfig) && ($data->pluginconfig->op ?? NULL) === "action") {
        $message = $this->t('Attempting to process SET @setid with action @action for ADO UUIDs @uuids.',
          [
            ...
            '@action' => $data->info['action'] ?? 'Undefined Action',
        ...
        // Actions do not define an AMI set status. So won't invoke a change.
        return;
      }
    }
    catch (\Throwable $exception) {
      $message = $this->t('Error Processing action @action for ADO UUIDs @uuids for set @setid with unexpected error @error. Skipping',
        [
          '@setid' => $data->info['set_id'] ?? 'Undefined AMI set ID',
          '@action' => $data->info['action'] ?? 'Undefined Action',
          '@uuids' => implode(",", $data->info['uuids'] ?? []),
    ...
    // try for normal ADO processing actions (update/ingest)
    try {
      $file_csv_columns = [];
          ...
          foreach ($parents as $parent_property => $parent_uuid) {
            ...
            if (count($existing) != count($parent_uuids)) {
              ...
              if ($data->info['attempt'] < 3) {
                ...
                // @TODO. We could keep track of the dependency
            ...
            foreach ($filenames as $filename) {
              ...
              if (empty($data->info['waiting_for_files'])) {
                ...
                if ($process_files_via_queue) {
                  ...
                  // Do the file processing synchronously
              ...
              if (!empty($data->info['waiting_for_files']) || !$process_files_via_queue) {
                ...
                if (!empty($processed_file_data['as_data']) && !empty($processed_file_data['file_id'])) {
                  ...
                  $file = $this->entityTypeManager->getStorage('file')
                    ->load($processed_file_data['file_id']);
                  if ($file) {
                    $file->setTemporary();
                    $file->save();
                  ...
                  if (is_numeric($data->info['set_id'])) {
      ...
      // NO need to check if $data->info['set_id'] is set. that is done inside ::setStatus
      // allowing a general/general watchdog log to be generated.
      $this->setStatus(amiSetEntity::STATUS_PROCESSING_WITH_ERRORS, $data);
      return;
    }
 ...
 try {
   ...
   if ($jsonstring) {
     ...
     try {
       ...
       $message = $this->t('Sorry we did all right but failed @ophuman the ADO with UUID @uuid on Set @setid. Something went wrong with error @e. Please check your Drupal Logs and notify your admin.', [
         '@uuid' => $data->info['row']['uuid'] ?? 'Undefined UUID',
         '@setid' => $data->info['set_id'] ?? 'Undefined AMI set ID',
         '@ophuman' => $op == $op_original ? static::OP_HUMAN[$op] : static::OP_HUMAN[$op] . ' and ' . static::OP_HUMAN[$op_original],
     ...
     return FALSE;
      ...
      $message = $this->t('Sorry, something failed badly while we attempted to @ophuman the ADO with UUID @uuid on Set @setid during the final persisting stage with error @error. This is quite strange. Please check your Drupal Logs and notify your admin.', [
        '@uuid' => $data->info['row']['uuid'] ?? 'Undefined UUID',
        '@setid' => $data->info['set_id'] ?? 'Undefined AMI set ID',
    ...
    try {
      ...
      if (($data->info['force_file_process'] ?? FALSE)) {
        ...
        if ($file) {
          ...
          $message = $this->t('Sorry, we really tried to process File @filename from Set @setid. Giving up.',
            [
  ...
  private function canProcess($data): bool|null
  {
    ...
    */
    try {
      ...
      $message = $this->t('Sorry. @ophuman ADO with UUID @uuid via Set @setid could not be validated because of an unexpected error @error. Skipping.', [
        '@uuid' => $data->info['row']['uuid'] ?? 'Undefined UUID',
        '@setid' => $data->info['set_id'] ?? 'Undefined AMI set ID',
        '@ophuman' => static::OP_HUMAN[$op],
        '@error' => $e->getMessage()
  ...
  private function processAction($data): bool|null {
    ...
    try {
      ...
      $message = $this->t('Error attempting to switch to the user account that sent the processing for Set @setid with unexpected error @error. Bailing out.', [
        '@setid' => $data->info['set_id'] ?? 'Undefined AMI set ID',
        '@error' => $e->getMessage(),
      ]);
      $this->loggerFactory->get('ami_file')->error($message, [
    ...
    try {
      ...
      $message = $this->t('Error loading ADOs with UUID @uuid for @action via Set @setid with unexpected error @error. ADO(s() in this queue item will be skipped. ', [
        '@uuid' => $data->info['uuids'] ?? 'Unknown UUID',
        '@setid' => $data->info['set_id'] ?? 'Undefined AMI set ID',
        '@action' => $data->info['action'],
        '@error' => $e->getMessage(),
    ...
    if (($data->info['action'] ?? NULL) == 'delete') {
      if (count($existing)) {
        ...
        try {
          ...
          $message = $this->t('Error executing @action on ADOs via Set @setid with unexpected error @error. Skipping.', [
            '@setid' => $data->info['set_id'] ?? 'Undefined AMI set ID',
            '@action' => $data->info['action'],
            '@error' => $e->getMessage(),
          ]);
    ...
    elseif (!empty($data->info['action'])) {
      try {
        ...
        // The VBO delete action decided to use the View in the logging. Breaks everything
        // We will create a "fake" empty View that is never executed.
        if ($action && \method_exists($action, 'setView')) {
          $viewconfig = [];
          $viewconfig['display']['default'] = [
            'id' => 'ami_set_'. $data->info['set_id'] ?? 'ami_set_default',
            'display_plugin' => 'default',
            'display_title' => 'Default',
          ];
          $viewconfig['id'] = 'ami_set_'. $data->info['set_id'] ?? 'unknown';
          $fakeview = new View($viewconfig, 'view');
          $action->setView($fakeview->getExecutable());
        }

        if ($action && \method_exists($action, 'setContext')) {
          $context_keystore_id = 'ami_action_' . $data->info['set_id'] . '_' . $data->info['time_submitted'];
          // We set current_batch to 0, so we can increment upfront?
          ...
          if (empty($context['sandbox']['total'])) {
            ...
            $context['view_id'] = 'ami_set_'. $data->info['set_id'];
        ...
        // If the action uses $context we need to run it any ways, even if
        // no entities were found or none can be accessed, or any logic that depends on
        ...
        $message = $this->t('Error. The action @action configured to run on Set @setid does not exist or failed because of an unexpected error @error. Skipping.', [
          '@setid' => $data->info['set_id'] ?? 'Undefined AMI set ID',
          '@action' => $data->info['action'],
          '@error' => $e->getMessage(),
        ]);
