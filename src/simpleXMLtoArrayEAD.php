...
class simpleXMLtoArrayEAD {
  ...
  public function xmlToArray(SimpleXMLElement $element = null) {
    // This here will allow us to reconstruct from JSON bc the position of the inner tags
    // relative to a piece of text won't be preserved.
    // Idea. If a tag has both data (text) and tags inside. Don't iterate. Just return the XML. We will treat as HTML
    // Later on.
    if ($element === null) {
      $element = $this->subject;
    }

    $namespaces_doc = $element->getDocNamespaces();
    $namespaces =  $element->getNamespaces();
    $namespaces = array_unique(array_merge($namespaces,$namespaces_doc));

    if (!isset($namespaces[''] )) {
      $namespaces['']  = null;
    }
    // Local override for Bad bad EAD.
    $namespaces[''] = null;
    ...
    if (($plainText !== '') && count($tagsArray)) {
      $plainText = $element->asXML();
      $local_tag = $element->getName();
      $plainText = preg_replace("!^<{$local_tag}(\s*?.*?)*?>(.*?)</{$local_tag}>$!is", '$2', $plainText);
    }
